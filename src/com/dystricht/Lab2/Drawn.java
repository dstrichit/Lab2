 ...
 *  @author Kevin Wayne
 */
public final class Drawn implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    ***************************************************************************/
    ...
    private static BufferedImage getImage(String filename) {
        ...
        try {
            URL url = Drawn.class.getResource(filename);
            BufferedImage image = ImageIO.read(url);
            return image;
        } 
        catch (IOException e) {
   ...
   /**
    ...
    public static void main(String[] args) {
        ...
        Drawn draw1 = new Drawn("Test client 1");
        draw1.square(.2, .8, .1);
        draw1.filledSquare(.8, .8, .2);
        draw1.circle(.8, .2, .2);
        draw1.setPenColor(Drawn.MAGENTA);
        draw1.setPenRadius(.02);
        draw1.arc(.8, .2, .1, 200, 45);


        // create another one
        Drawn draw2 = new Drawn("Test client 2");
        draw2.setCanvasSize(900, 200);
        // draw a blue diamond
        draw2.setPenRadius();
        draw2.setPenColor(Drawn.BLUE);
        double[] x = { .1, .2, .3, .2 };
        double[] y = { .2, .3, .2, .1 };
        draw2.filledPolygon(x, y);

        // text
        draw2.setPenColor(Drawn.BLACK);
        draw2.text(0.2, 0.5, "bdfdfdfdlack text");
        draw2.setPenColor(Drawn.WHITE);
